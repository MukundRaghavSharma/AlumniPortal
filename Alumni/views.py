...
def family_trees_create(request):
    ...
    for family in families:
        ...
        for alumni in Alumni.objects.filter(family = family):
            ...
            little_name = alumni.user.first_name + ' ' + alumni.user.last_name + "</a>'"
            big_text = '\'\',\'\'],'
            if big != None:
                big_name = big.user.first_name + ' ' + big.user.last_name
                big_name = big.user.first_name + ' ' + big.user.last_name + '</a>'
                big_text = big_name + '\',' + '\'\'],'   
                big_text = '<a href="/profile/' + str(big.number) + '"' + big_name + "', '']," 
                #big_text = big_name + '\',' + '\'\'],'   
            little_text = "['<a href=\"/profile/" + str(alumni.number) + '"' + little_name + ",'" 
